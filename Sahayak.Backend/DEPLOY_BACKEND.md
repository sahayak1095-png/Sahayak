...
5) If you have a custom backend domain, add it in the Render dashboard or provide it via `RENDER_BACKEND_URL` to the `deploy-full` workflow. For your domain, use `https://api.saha-yak.in` and configure DNS for `api.saha-yak.in` to point to Render's provided CNAME/A records.
